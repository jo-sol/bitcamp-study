...
public class App {
  ...
  // boardList~projectList 뭐가 담긴지 몰라서 <E> // <E> -> 클래스가 아니라 데이터 타입이라는 걸 지정 
  @SuppressWarnings("unchecked")
  private <E> void loadObjects(String filepath, List<E> list) { 
    try (ObjectInputStream in = new ObjectInputStream(
        new BufferedInputStream(
            new FileInputStream(filepath)))) {
      // 어떤 데이터를 호출할 건지 아직 모르지만, ( <E> )
      // 메서드를 호출하는 순간 타입이 지정됨

      // 파일에서 읽은 데이터를 list에 담아 줌
  ...
  private <E> void saveObjects(String filepath, List<E> list) { // <E>은 제네릭 타입 파라미터 >> 아직 뭔지 모름
