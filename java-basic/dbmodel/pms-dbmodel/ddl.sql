...
ALTER TABLE pms_mylist
  ADD CONSTRAINT PK_pms_mylist -- 내관심게시글(즐겨찾기) 기본키
    PRIMARY KEY (
      member_no, -- 회원번호
      board_no   -- 게시글번호
    );
...
-- 내관심게시글(즐겨찾기)
ALTER TABLE pms_mylist
  ADD CONSTRAINT FK_pms_member_TO_pms_mylist -- 회원 -> 내관심게시글(즐겨찾기)
    FOREIGN KEY (
      member_no -- 회원번호
    )
    REFERENCES pms_member ( -- 회원
...
-- 내관심게시글(즐겨찾기)
ALTER TABLE pms_mylist
  ADD CONSTRAINT FK_pms_board_TO_pms_mylist -- 게시판 -> 내관심게시글(즐겨찾기)
    FOREIGN KEY (
      board_no -- 게시글번호
    )
    REFERENCES pms_board ( -- 게시판
