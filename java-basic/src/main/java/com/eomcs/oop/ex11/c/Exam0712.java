// inner class 응용 I : 2단계 - 논스태틱 중첩 클래스 사용
...
class Musics2 {
  ...
  // - 생성자가 바깥 클래스의 인스턴스를 받는 파라미터가 자동으로 추가된다.
  class Player { // static class Player -> class Player [스태틱을 빼고 inner class로 만들면]
    ...
    public void play() {
      ...
      // 다음과 같이 '바깥클래스명.this.멤버(필드명)' 형식으로 접근한다.
