...
public class Exam03 {
  public static void main(String[] args) {
    A3 obj; // 1) 자식 클래스의 인스턴스도 저장

    // 추상 클래스의 인스턴스는 생성 불가!
    // => 만약 아래처럼 만든다면?
    //    obj = new A3(); // Error!
    // => A3에 m1이 없어서 실행할 수 없음! 둘 다 추상!!

    // 추상 메서드를 구현한 서브 클래스 만이 인스턴스 생성 가능!
    obj = new A3Sub(); // 2) 그래서 이게 가능
  ...
  //추상 클래스의 인스턴스는 생성할 수 없기 때문에 넘어오지 않는다.
  // A3 obj는 A3를 상속 받아서 만든 클래스의 m1을 호출하라는 의미
  static void test(A3 obj) { 
    ...
    // - 따라서 obj를 가지고 m1()을 호출할 때는
    //   실제 obj가 가리키는 인스턴스의 클래스에서 m1() 메서드를 찾아 호출한다.
    // - 다시 한번 말하지만 인스턴스를 만들었다는 것은
    //   일반 클래스라는 뜻이고,
    //   일반 클래스에는 추상 메서드가 없다.
