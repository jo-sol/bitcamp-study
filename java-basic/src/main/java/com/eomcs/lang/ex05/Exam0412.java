...
public class Exam0412 {
  public static void main(String[] args) {
    ...
    // 같은 값이 나오는 이유?
    // => int 타입의 값에 대해 비트 이동을 할 때는 0 ~ 31까지만 유효하다. (4바이트 정수일 때는 최대 31비트까지 이동 가능)
    //    만약 31을 넘는 경우 32로 나눈 나머지 값을 비트 이동으로 간주한다. (나머지 값을 비트 이동으로 간주한다 중요!)
    // => long 타입의 경우 비트 이동은 0 ~ 63까지 유효하다. (8바이트 정수일 때는 최대 63비트까지 이동 가능)
    //    만약 63을 넘는 경우 64로 나눈 나머지 값을 비트 이동으로 간주한다. (나머지 값을 비트 이동으로 간주한다 중요!)
    //
    // => 공식:
    //    n << s
