...
public class MenuGroup extends Menu {
  ...
  // **모든 메뉴(인스턴스)가 공유할 객체이기 때문에 스태틱 멤버로 선언한다.
  ...
  public void execute() {
    ...
    breadCrumb.push(this); // this = 메뉴 객체(현재 실행하는 메뉴를 스택에 보관)
    ...
    while (true) {
      ...
      if (menuNo == 0 && !disablePrevMenu) { // 0 번이면 return하게 되어 있음
  ...
  private String getBreadCrumb() {
    ...
    for (int i = 0; i < breadCrumb.size(); i++) { 
      if (path.length() > 0) { // path 값이 0보다 크다면 /를 붙임
        path += " / ";
      }
      Menu menu = (Menu) breadCrumb.get(i); // breadCrumb의 i는 Menu
      path += menu.title;
    }
