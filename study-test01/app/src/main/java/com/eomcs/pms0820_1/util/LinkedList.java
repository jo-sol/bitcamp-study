...
public class LinkedList<E> extends AbstractList<E> {
  ...
  public void add(E item) {
    ...
    // ***레퍼런스에 Node에 전달하는 타입이 뭔지에 대해 설정해 주는 <E> 정보가 있을 때는
    // ***객체를 생성해 줄 때 E 를 생략할 수 있다.
    // ***그러나 <>는 생략하면 안 된다!
  ...
  @Override
  public E get(int index) { // 수퍼 클래스를 상속받은 후, 추상메서드를 제대로 구현했는지 Override로 검사해달라 요청
    ...
    for (int i = 0; i < this.size; i++) {
      ...
      node = node.next; // 그게 아니라면 현재 node가 가리키고 있는 다음으로 이동
    }
    return null; // 반복문이 끝날 때까지 찾지 못했으면 null 리턴
  }
  ...
  public E remove(int index) {
    ...
    for (int i = 0; i < this.size; i++) {// 반복문 들어가고 값을 찾아야 함
      ...
      if (i == index) { // 삭제할 위치를 찾았다면
        ...
        // 가비지 관리를 위해 삭제할 노드의 인스턴스 필드를 null로 초기화한다.
        node.item = null; // 삭제했는데 그 항목을 가리키면 가비지가 안 되니까 같이 null로 초기화
        node.next = null;
