...
public class Controller01_1 { // 페이지 컨트롤러

  @RequestMapping // 이 애노테이션을 붙여서 요청이 들어왔을 때 호출될 메서드임을 표시한다.
  @ResponseBody // 메서드의 리턴 값이 클라이언트에게 출력할 내용임을 표시한다.
  public String handler() { // 요청이 들어왔을 때 처리하는 메서드 -> 요청을 다루는 자(처리하는 자) requestHandler
  ...
  //@RequestMapping
  @ResponseBody
  public String handler2() { // 요청이 들어왔을 때 처리하는 메서드 -> 요청을 다루는 자(처리하는 자) requestHandler
